...
if(file.exists("C:/Users/am4337/Documents/R/coursera/har.RDATA")){
    load(file = "C:/Users/am4337/Documents/R/coursera/har.RDATA", verbose = FALSE)
} else{
...
validation <- df[whichset == 2, ]
testing <- df[whichset == 3, ]
```
...
It looks like there are a few highly correlated predictors. Using PCA pre-processing with a threshold of .98 will only drop us down 3 variables, and hopefully reducing some noise in the data. After PCA pre-processing, we can build a linear model.

```{r GLM, eval = FALSE}

preProc <- preProcess(training[, colnames(training) != "classe"], method = "pca", thresh = .98)
trainPC <- predict(preProc, training[, colnames(training) != "classe"])
model.lda <- train(training$classe ~ ., method = "lda", data = trainPC)

preProc.validate <- preProcess(validation[, colnames(validation) != "classe"], method = "pca", thresh = .98)
validPC <- predict(preProc.validate, validation[, colnames(training) != "classe"])
confmat <- confusionMatrix(validation$classe, predict(model.lda, validPC))
...
confusionMatrix(validation$classe, predict(model.rpart, validation))
```

The tree did better than lda, but still the accuracy is not very good, especially for class C.

Instead, we can go with a random forest model, which my computer can handle. Random Forest also does not require preprocessing, as it can handle co-linear variables without a problem.
...
Much better than the linear discriminant model! Let's do our final cross-validation with the testing data set.

```{r validate}
confusionMatrix(testing$classe, predict(model.rf, testing))
```

Still good accuracy! Let's get the final predictions on the test set. We'll have to clean the test data the same way we did the training & validation data, and then predict.
